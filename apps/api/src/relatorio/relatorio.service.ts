...
export class RelatorioService {
  ...
  async gerarPptx(experimentoId: string, user: UsuarioAtual): Promise<Buffer> {
    ...
    capa.addText(
      [exp.objetoEstudo?.nome, exp.local?.nome, exp.safra?.nome, exp.delineamento?.nome].filter(Boolean).join("  ·  "),
    ...
    const meta: [string, string][] = [
      ["Ensaio", exp.ensaio], ["Status", exp.status],
      ["Objeto de estudo", exp.objetoEstudo?.nome ?? "—"], ["Cultivar", exp.cultivar ?? "—"],
      ["Área de pesquisa", exp.areaPesquisa?.nome ?? "—"], ["Local", exp.local?.nome ?? "—"],
      ["Safra", exp.safra?.nome ?? "—"], ["Delineamento", exp.delineamento?.nome ?? "—"],
      ["Tratamentos", String(exp.numTratamentos ?? "—")], ["Repetições", String(exp.numRepeticoes ?? "—")],
    ...
    resumo.addTable(
      ...
      { x: 0.7, y: 1.4, w: 7, fontSize: 13, border: { type: "solid", color: "E1E1EF", pt: 1 }, rowH: 0.32 },
    );
    ...
    for (const aval of exp.avaliacoes) {
      ...
      this.titulo(slide, `Análise — ${aval.nome}${aval.unidadeSaida ? ` (${aval.unidadeSaida})` : ""}`);

      // Tabela ANOVA
      const head = ["Fonte", "GL", "SQ", "QM", "F", "p"].map((t) => ({ text: t, options: { bold: true, color: "FFFFFF", fill: { color: NAVY } } }));
      const linhas = r.tabela.map((l) => [
        { text: l.fonte }, { text: String(l.gl) }, { text: l.sq.toFixed(2) }, { text: l.qm.toFixed(2) },
        { text: l.f != null ? l.f.toFixed(2) : "—" },
        { text: l.p != null ? (l.p < 0.001 ? "<0.001" : l.p.toFixed(3)) : "—" },
      ]);
      slide.addTable([head, ...linhas], { x: 0.7, y: 1.4, w: 6.6, fontSize: 12, border: { type: "solid", color: "E1E1EF", pt: 1 }, rowH: 0.3 });
      slide.addText(
      ...
      const headM = ["Trat.", "Média", ""].map((t) => ({ text: t, options: { bold: true, color: "FFFFFF", fill: { color: GREEN } } }));
      const linhasM = r.medias.map((m) => [
        { text: m.tratamento, options: { bold: true } }, { text: m.media.toFixed(1) }, { text: m.letra ?? "", options: { bold: true, color: "2D6CDF" } },
      ]);
      slide.addTable([headM, ...linhasM], { x: 7.7, y: 1.4, w: 2.6, fontSize: 12, border: { type: "solid", color: "E1E1EF", pt: 1 }, rowH: 0.3 });

      // Gráfico de barras das médias
      slide.addChart(pptx.ChartType.bar, [{ name: aval.nome, labels: r.medias.map((m) => m.tratamento), values: r.medias.map((m) => Number(m.media.toFixed(2))) }], {
        x: 7.7, y: 4.0, w: 5.0, h: 3.0, showTitle: false, showLegend: false, barDir: "col", chartColors: [SKY],
  ...
  private titulo(slide: pptxgen.Slide, texto: string) {
    ...
    slide.addShape("line" as never, { x: 0.7, y: 1.2, w: 12, h: 0, line: { color: SKY, width: 2 } });
  }
}
