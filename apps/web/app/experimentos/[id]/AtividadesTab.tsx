...
export function AtividadesTab({ exp }: { exp: Experimento }) {
  ...
  async function adicionar() {
    ...
    setErro(null);
    try {
      await api.criarAtividadeExp(exp.id, { modeloId: sel });
      setSel("");
      recarregar();
    } catch (e) {
  ...
  async function gerarMarcos() {
    setErro(null);
    try {
      await api.gerarMarcos(exp.id);
      recarregar();
    } catch (e) {
      setErro(e instanceof Error ? e.message : "Falha ao gerar marcos");
    }
  }
    ...
    <div>
      ...
      >
        ...
        >
          ...
          <button
            onClick={gerarMarcos}
            style={{ ...btn("#4EC2F0"), marginLeft: "auto", color: "#1F2940" }}
          >
            Gerar marcos
          </button>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ color: "#7987A1", textAlign: "left" }}>
                {["Marco", "Previsão", "Confirmado", "Data realizada", ""].map((h) => (
            ...
            <tbody>
              {marcos.length === 0 && (
                <tr>
                  <td colSpan={4} style={{ padding: "8px 12px", color: "#a9abbd" }}>
                    Nenhum marco. Clique em “Gerar marcos” (semeadura/colheita aparecem se o objeto
                    for cultura).
                  </td>
                </tr>
              )}
              {marcos.map((m) => (
                <MarcoRow key={m.id} marco={m} onChange={recarregar} />
              ))}
            </tbody>
          </table>
        </div>
      ...
      >
        ...
        <button
          data-testid="atv-exp-add"
          onClick={adicionar}
          disabled={!sel}
          style={btn(sel ? "#1F2940" : "#a9abbd")}
        >
          adicionar
        </button>
        {modelos.length === 0 && (
          <span style={{ fontSize: 12, color: "#7987A1" }}>
            nenhum modelo no catálogo — cadastre em /catalogo
          </span>
        )}
      </div>

      {comuns.length === 0 && (
        <p style={{ color: "#a9abbd" }}>Nenhuma atividade (não-marco) registrada.</p>
      )}
      {comuns.map((a) => (
    ...
    <>
      <tr style={{ borderTop: "1px solid #eef0f5" }}>
        ...
        <td style={{ padding: "8px 12px" }}>
          <input
            ...
            value={prevista}
            onChange={(e) => {
              setPrevista(e.target.value);
              salvar({ dataPrevista: e.target.value || null });
            }}
            style={inp}
          />
        </td>
        ...
        <td style={{ padding: "8px 12px" }}>
          <input
            ...
            onChange={(e) => {
              ...
              salvar({ data: e.target.value || null });
            }}
            style={inp}
          />
        </td>
        <td style={{ padding: "8px 12px" }}>
          <button
            ...
            onClick={async () => {
              if (confirm(`Remover o marco "${marco.nome}"?`)) {
                await api.removerAtividadeExp(marco.id);
                onChange();
              }
            }}
            style={mini("#F34343")}
      ...
      {campos.length > 0 && (
        <tr style={{ background: "#f7fafd" }}>
          <td colSpan={5} style={{ padding: "4px 12px 10px 24px" }}>
            <MarcoMedicoes marco={marco} onChange={onChange} />
          </td>
        </tr>
      )}
    </>
...
function MarcoMedicoes({ marco, onChange }: { marco: AtividadeExperimento; onChange: () => void }) {
  ...
  const [vals, setVals] = useState<Record<string, string>>(() => {
    ...
    for (const c of campos)
      init[c.rotulo] = (
        marco.valores.find((v) => v.rotulo === c.rotulo)?.valorNum ?? ""
      ).toString();
    return init;
  });
  ...
  async function salvar() {
    ...
    setErro(null);
    try {
      await api.registrarApontamento(
        marco.id,
        campos.map((c) => ({
          rotulo: c.rotulo,
          valorNum: vals[c.rotulo] === "" ? null : Number(vals[c.rotulo]),
        })),
      );
      setMsg("medições salvas");
      onChange();
    } catch (e) {
      setErro(e instanceof Error ? e.message.replace(/^\d+\s*/, "") : "falha");
    }
  }
      ...
      {campos.map((c) => (
        ...
        >
          ...
          <input
            type="number"
            value={vals[c.rotulo] ?? ""}
            onChange={(e) => setVals({ ...vals, [c.rotulo]: e.target.value })}
            style={{ ...inp, width: 80 }}
          />
        </label>
      ))}
...
function AtividadeCard({
  ...
  atividade: AtividadeExperimento;
  onChange: () => void;
}) {
  ...
  async function salvar() {
    ...
    try {
      await api.registrarApontamento(atividade.id, valores);
      setMsg("Apontamento salvo.");
      onChange();
    } catch (e) {
      setErro(e instanceof Error ? e.message.replace(/^\d+\s*/, "") : "Falha ao salvar");
    }
  }

  async function remover() {
    if (!confirm(`Remover a atividade "${atividade.nome}"?`)) return;
    await api.removerAtividadeExp(atividade.id);
    onChange();
  }
      ...
      {atividade.tipo === "apontamento" && campos.length > 0 && (
        <div style={{ padding: 14 }}>
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 10, alignItems: "center" }}>
            <button data-testid="atv-apont-salvar" onClick={salvar} style={btn("#1F2940")}>
              Salvar apontamento
            </button>
            {msg && <span style={{ fontSize: 12, color: "#1F2940" }}>{msg}</span>}
            {erro && <span style={{ fontSize: 12, color: "#F34343" }}>{erro}</span>}
