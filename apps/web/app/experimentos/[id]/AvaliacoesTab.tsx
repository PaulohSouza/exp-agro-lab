...
export function AvaliacoesTab({
  ...
  const [modo, setModo] = useState<
    | { tipo: "lista" }
    | { tipo: "lancar"; aval: Avaliacao }
    | { tipo: "relatorio"; aval: Avaliacao }
    | { tipo: "analise"; aval: Avaliacao }
    | { tipo: "lote" }
  >({ tipo: "lista" });
  ...
  if (modo.tipo === "relatorio")
    return <Relatorio aval={modo.aval} voltar={() => setModo({ tipo: "lista" })} />;
  if (modo.tipo === "analise")
    return <Analise aval={modo.aval} voltar={() => setModo({ tipo: "lista" })} />;
  if (modo.tipo === "lote")
    return (
      <ColetaLote
        exp={exp}
    ...
    <div>
      ...
      {avaliacoes.length > 0 && (
        <div style={{ marginTop: 10 }}>
          <button
            data-testid="abrir-coleta-lote"
            onClick={() => setModo({ tipo: "lote" })}
            style={mini("#1F2940")}
          >
            Coleta em lote (grade)
          </button>
        </div>
      )}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 16 }}>
          <thead>
            <tr style={{ background: "#1F2940", color: "#fff", textAlign: "left" }}>
              {["Seq", "Avaliação", "Tipo", "Unid. saída", "Timing", "Lançamentos", "Ações"].map(
                (h) => (
                  <th key={h} style={th}>
                    {h}
                  </th>
          ...
          <tbody>
            {avaliacoes.map((a, i) => (
              <tr key={a.id} style={{ borderBottom: "1px solid #eaecf3" }}>
                ...
                <td style={td}>
                  <button
                    onClick={() => setModo({ tipo: "lancar", aval: a })}
                    style={mini("#6FA830")}
                  >
                    Lançar
                  </button>{" "}
                  {a.formula && (
                    <button
                      onClick={() => setModo({ tipo: "relatorio", aval: a })}
                      style={mini("#4EC2F0")}
                    >
                      Relatório
                    </button>
            ...
            {avaliacoes.length === 0 && (
              <tr>
                <td style={td} colSpan={7}>
                  <span style={{ color: "#a9abbd" }}>Nenhuma avaliação.</span>
                </td>
              </tr>
            )}
          </tbody>
...
function AdicionarDoCatalogo({ exp, onAdicionou }: { exp: Experimento; onAdicionou: () => void }) {
  ...
  async function adicionar() {
    ...
    try {
      ...
      let txt = nomes.length
        ? `Adicionada(s): ${nomes.join(", ")}.`
        : "Modelo já estava no experimento.";
      if (r.prerequisitosAdicionados.length)
        txt += ` Avaliação(ões) pré-requisito incluída(s): ${r.prerequisitosAdicionados.join(", ")}.`;
      if (r.atividadesAdicionadas.length)
        txt += ` Atividade(s) pré-requisito incluída(s) (ver aba Atividades): ${r.atividadesAdicionadas.join(", ")}.`;
      setMsg(txt);
      setSel("");
      onAdicionou();
    } catch (e) {
  ...
  return (
    ...
    >
      <span style={{ fontSize: 13, color: "#1F2940", fontWeight: 600 }}>
      ...
      <select
        ...
        value={filtro}
        onChange={(e) => {
          setFiltro(e.target.value as EscopoModelo | "todos");
          setSel("");
        }}
        style={inp}
      >
      ...
      >
        ...
        {visiveis.map((m) => (
          <option key={m.id} value={m.id}>
            [{ESCOPO_INFO[m.escopo].sigla}] {m.nome}
            {m.unidadeSaida ? ` (${m.unidadeSaida})` : ""}
          </option>
        ))}
      </select>
      <button
        title="Ver metodologia"
        aria-label="Ver metodologia"
        disabled={!selModelo}
        onClick={() => setInfo(selModelo)}
        style={mini(selModelo ? "#4EC2F0" : "#a9abbd")}
      >
        ⓘ info
      </button>
      <button
        data-testid="add-catalogo-btn"
        onClick={adicionar}
        disabled={!sel}
        style={mini(sel ? "#1F2940" : "#a9abbd")}
      >
        adicionar
      </button>
      {msg && (
        <span data-testid="add-catalogo-msg" style={{ fontSize: 12, color: "#1F2940" }}>
          {msg}
        </span>
      )}
    ...
    >
      ...
      >
        ...
        <div style={{ padding: 18, fontSize: 13, color: "#1F2940", display: "grid", gap: 10 }}>
          ...
          <Linha
            rotulo="Unidade"
            valor={`${modelo.unidadeColeta ?? "—"}${modelo.unidadeSaida ? ` → ${modelo.unidadeSaida}` : ""}`}
          />
          {modelo.calculoRelatorio && (
            <Linha rotulo="Cálculo (relatório)" valor={modelo.calculoRelatorio} />
          )}
          {modelo.prerequisitos && modelo.prerequisitos.length > 0 && (
            <Linha
              rotulo="Pré-requisitos (avaliações)"
              valor={modelo.prerequisitos.map((p) => p.prerequisito.nome).join(", ")}
            />
          )}
          {modelo.prerequisitosAtividade && modelo.prerequisitosAtividade.length > 0 && (
            <Linha
              rotulo="Pré-requisitos (atividades)"
              valor={modelo.prerequisitosAtividade.map((p) => p.modeloAtividade.nome).join(", ")}
            />
          )}
...
function Linha({ rotulo, valor }: { rotulo: string; valor: string }) {
  return (
    <div>
      <span style={{ color: "#7987A1" }}>{rotulo}: </span>
      <strong>{valor}</strong>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ whiteSpace: "pre-wrap" }}>
        {texto?.trim() ? texto : <span style={{ color: "#a9abbd" }}>— não informado —</span>}
      </div>
    </div>
  );
}
...
function AplicarGrupo({ exp, onAplicou }: { exp: Experimento; onAplicou: () => void }) {
  ...
  async function aplicar() {
    ...
    try {
      ...
      const nomes = r.criadas.map((a) => a.nome);
      let txt = nomes.length
        ? `Grupo aplicado — adicionada(s): ${nomes.join(", ")}.`
        : "Avaliações do grupo já estavam no experimento.";
      if (r.atividadesAdicionadas.length)
        txt += ` Atividade(s): ${r.atividadesAdicionadas.join(", ")}.`;
      setMsg(txt);
      setSel("");
      onAplicou();
    } catch (e) {
      setErro(e instanceof Error ? e.message : "Falha ao aplicar grupo");
    }
  }
  ...
  return (
    ...
    >
      <span style={{ fontSize: 13, color: "#1F2940", fontWeight: 600 }}>
        Aplicar grupo de coleta:
      </span>
      <select
        data-testid="aplicar-grupo-select"
        value={sel}
        onChange={(e) => setSel(e.target.value)}
        style={inp}
      >
        ...
        {grupos.map((g) => (
          <option key={g.id} value={g.id}>
            {g.nome} ({g.itens?.length ?? 0})
          </option>
        ))}
      </select>
      <button
        data-testid="aplicar-grupo-btn"
        onClick={aplicar}
        disabled={!sel}
        style={mini(sel ? "#1F2940" : "#a9abbd")}
      >
...
function ColetaLote({ exp, voltar }: { exp: Experimento; voltar: () => void }) {
  ...
  const parcelas = useMemo(
    () => [...(exp.parcelas ?? [])].sort((a, b) => a.numero - b.numero),
    [exp.parcelas],
  );
  ...
  useEffect(() => {
    api
      ...
      .then(setGrupos)
      .catch(() => {});
    Promise.all(avals.map((a) => api.listarDados(a.id).then((ds) => [a.id, ds] as const))).then(
      (pares) => {
        const m: Record<string, Record<string, string>> = {};
        for (const [avalId, ds] of pares)
          for (const d of ds) (m[d.parcelaId] ??= {})[avalId] = d.valorColetado?.toString() ?? "";
        setVals(m);
      },
    );
  ...
  async function salvar() {
    ...
    for (const p of parcelas)
      for (const a of filtradas) {
        ...
        if (v !== undefined && v !== "")
          lancamentos.push({ avaliacaoId: a.id, parcelaId: p.id, valorColetado: Number(v) });
      }
  ...
  return (
    <div>
      ...
      >
        ...
        <label style={{ fontSize: 12, color: "#7987A1" }}>
          Timing:
          <select
            value={timing}
        ...
        <button data-testid="coleta-lote-salvar" onClick={salvar} style={mini("#1F2940")}>
          Salvar tudo
        </button>
        {msg && <span style={{ fontSize: 12, color: "#1F2940" }}>{msg}</span>}
      </div>
      {filtradas.length === 0 ? (
        <p style={{ color: "#a9abbd" }}>Nenhuma avaliação no filtro.</p>
      ) : (
        <div className="tabela-scroll">
              ...
              <tr style={{ background: "#1F2940", color: "#fff", textAlign: "left" }}>
                ...
                {filtradas.map((a) => (
                  <th key={a.id} style={th}>
                    {a.nome}
                    {a.unidadeColeta ? ` (${a.unidadeColeta})` : ""}
                  </th>
                ))}
              </tr>
            </thead>
                ...
                <tr key={p.id} style={{ borderBottom: "1px solid #f0f0f8" }}>
                  ...
                  {filtradas.map((a) => (
                    <td key={a.id} style={td}>
                      <input
                        value={vals[p.id]?.[a.id] ?? ""}
                        onChange={(e) => set(p.id, a.id, e.target.value)}
                        style={{ ...inp, width: 80 }}
                      />
                    </td>
                  ))}
...
function NovaAvaliacao({ exp, onCriou }: { exp: Experimento; onCriou: () => void }) {
  ...
  return (
    ...
    >
      ...
      <button type="submit" style={mini("#6FA830")}>
        Nova avaliação
      </button>
    </form>
  );
  ...
  return (
    <div>
      ...
      {usaArea && (
        ...
        >
          ℹ️ A área útil para o cálculo (kg/ha) vem da atividade <strong>Colheita</strong> (nº de
    ...
    <div>
      <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
        ...
        <span style={{ color: "#7987A1", fontSize: 13 }}>
          conversão {aval.unidadeColeta} → {aval.unidadeSaida} (calculada no relatório)
        </span>
      </div>
              ...
              <table style={{ borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ color: "#7987A1", textAlign: "left" }}>
                    ...
                    <th style={th}>{aval.unidadeSaida}</th>
                  </tr>
                </thead>
                <tbody>
                  {rel.linhas.map((l, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid #f0f0f8" }}>
                      ...
                      <td style={td}>{l.tratamento}</td>
                      <td style={td}>{l.valorColetado ?? "—"}</td>
                      <td style={td}>{l.areaUtilM2 ?? "—"}</td>
                      <td style={td}>
                        <strong>{l.valorSaida != null ? Math.round(l.valorSaida) : "—"}</strong>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <table style={{ borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ color: "#7987A1", textAlign: "left" }}>
                    <th style={th}>Trat.</th>
                    <th style={th}>Nome</th>
                    <th style={th}>Média ({aval.unidadeSaida})</th>
                  </tr>
                </thead>
                <tbody>
                  {rel.medias.map((m) => (
                    <tr key={m.tratamento} style={{ borderBottom: "1px solid #f0f0f8" }}>
                      ...
                      <td style={td}>{Math.round(m.media)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <p style={{ color: "#a9abbd", fontSize: 11, marginTop: 8 }}>
              Médias simples (a análise estatística completa vem no Marco 4).
            </p>
          </div>
        </div>
      )}
        ...
        <div style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#1F2940", marginTop: 8 }}>
              CV = <strong>{a.resultado.cv.toFixed(2)}%</strong> · média geral{" "}
              {a.resultado.mediaGeral.toFixed(1)} ·{" "}
              {a.resultado.significativo ? (
                <span style={{ color: "#6FA830" }}>tratamento significativo</span>
              ) : (
          ...
          <div>
            ...
            <p style={{ color: "#a9abbd", fontSize: 11, marginTop: 8, maxWidth: 240 }}>
              Letras iguais = sem diferença significativa (α={a.resultado.comparacao.alpha}).
              Tukey/Scott-Knott/LSD portados do SAGRE; validação golden vs saída do R pendente.
            </p>
          </div>
        </div>
      )}
