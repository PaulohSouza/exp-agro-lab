...
export function GeralTab({
  ...
  const [refs, setRefs] = useState<{
    ...
    delineamentos: Ref[];
    categorias: Ref[];
  }>({ locais: [], safras: [], areas: [], delineamentos: [], categorias: [] });
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.delineamentos(),
      api.categorias(),
    ]).then(([locais, safras, areas, delineamentos, categorias]) =>
      setRefs({ locais, safras, areas, delineamentos, categorias }),
    );
  }, []);
  useEffect(() => {
    if (cat) api.subcategorias(cat).then(setSubs);
    else setSubs([]);
  }, [cat]);
  useEffect(() => {
    if (sub) api.objetos(sub).then(setObjs);
    else setObjs([]);
  }, [sub]);
    ...
    <div>
      ...
      >
        <span style={{ color: "#7987A1", fontSize: 13 }}>
          Edite os dados do experimento. O delineamento dirige o croqui.
        </span>
        <span>
          <Link href="/cadastros" style={{ color: "#2D6CDF", fontSize: 13, marginRight: 12 }}>
            gerenciar cadastros →
          </Link>
      ...
      <div style={grid}>
        <Campo label="Título">
          <input style={inp} value={f.titulo} onChange={(e) => up("titulo", e.target.value)} />
        </Campo>
        <Campo label="Código">
          <input
            style={inp}
        ...
        <Campo label="Área de pesquisa">
          <Select v={f.areaPesquisaId} set={(v) => up("areaPesquisaId", v)} opts={refs.areas} />
        </Campo>
        <Campo label="Local">
          <Select v={f.localId} set={(v) => up("localId", v)} opts={refs.locais} />
        </Campo>
        <Campo label="Período">
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {f.tipoPeriodo === "SAFRA" ? (
              ...
              <input
                ...
                value={f.anoSemestre}
                onChange={(e) => up("anoSemestre", e.target.value)}
                style={{
                  padding: 8,
                  borderRadius: 6,
        ...
        <Campo label="Delineamento">
          <Select
            v={f.delineamentoId}
            set={(v) => up("delineamentoId", v)}
            opts={refs.delineamentos}
          />
        </Campo>
        ...
        <Campo label="Cultivar / variedade">
          <input style={inp} value={f.cultivar} onChange={(e) => up("cultivar", e.target.value)} />
        </Campo>
      </div>
      ...
      <fieldset style={fs}>
        ...
        <div style={grid}>
          ...
          <Campo label="Objeto">
            <Select v={objetoId} set={setObjetoId} opts={objs} ph="— objeto —" />
          </Campo>
        </div>
      </fieldset>
      ...
      <fieldset style={fs}>
        ...
        <div style={grid}>
          <Campo label="Largura (m)">
            <input
              style={inp}
              value={f.parcelaLarguraM}
              onChange={(e) => up("parcelaLarguraM", e.target.value)}
            />
          </Campo>
          <Campo label="Comprimento (m)">
            <input
              style={inp}
              value={f.parcelaComprimentoM}
              onChange={(e) => up("parcelaComprimentoM", e.target.value)}
            />
          </Campo>
          ...
          <Campo label="Espaçamento linhas (m)">
            <input
              style={inp}
              value={f.espacamentoLinhasM}
              onChange={(e) => up("espacamentoLinhasM", e.target.value)}
            />
          </Campo>
        </div>
      </fieldset>
      ...
      <Campo label="Objetivo">
        <textarea style={ta} value={f.objetivo} onChange={(e) => up("objetivo", e.target.value)} />
      </Campo>
      <Campo label="Metodologia">
        <textarea
          style={ta}
          value={f.metodologia}
          onChange={(e) => up("metodologia", e.target.value)}
        />
      </Campo>
