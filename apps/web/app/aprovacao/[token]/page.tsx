  ...
  return (
    <main style={{ minHeight: "100vh", display: "grid", placeItems: "center", background: "#141B2D" }}>
      <div style={{ background: "#fff", borderRadius: 12, padding: 28, width: 420 }}>
        ...
        {resultado ? (
          <div>
            <p style={{ color: "#1F2940" }}>Experimento: <strong>{resultado.experimento}</strong></p>
            <p style={{ color: resultado.decisao === "aprovado" ? "#6FA830" : "#F34343", fontWeight: 600 }}>
          ...
          <div>
            <p style={{ color: "#7987A1", fontSize: 14 }}>Revise e registre sua decisão sobre a OS.</p>
            <textarea placeholder="Observação (opcional)" value={motivo} onChange={(e) => setMotivo(e.target.value)}
              style={{ width: "100%", minHeight: 60, padding: 8, borderRadius: 8, border: "1px solid #d6d6e6", boxSizing: "border-box", marginBottom: 12 }} />
            <div style={{ display: "flex", gap: 8 }}>
              <button disabled={busy} onClick={() => decidir("aprovado")} style={{ flex: 1, background: "#6FA830", color: "#fff", border: "none", borderRadius: 8, padding: "10px", cursor: "pointer" }}>Aprovar</button>
              <button disabled={busy} onClick={() => decidir("recusado")} style={{ flex: 1, background: "#F34343", color: "#fff", border: "none", borderRadius: 8, padding: "10px", cursor: "pointer" }}>Recusar</button>
            </div>
