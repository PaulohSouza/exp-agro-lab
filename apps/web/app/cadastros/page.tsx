...
function ListaSimples({
  ...
  titulo: string;
  tipo: "locais" | "safras" | "areas" | "delineamentos";
  carregar: () => Promise<Ref[]>;
}) {
  ...
  return (
    <div style={card}>
      ...
      <ul style={{ margin: "0 0 10px", paddingLeft: 18, color: "#1F2940" }}>
        ...
        {itens.length === 0 && (
          <li style={{ color: "#a9abbd", listStyle: "none", marginLeft: -18 }}>vazio</li>
        )}
      </ul>
  ...
  return (
    <div style={{ ...card, marginTop: 16 }}>
      ...
      <p style={{ color: "#7987A1", fontSize: 13, marginTop: 0 }}>
        Ex.: Cultura → Algodão → "FM 944 GL"; Máquina → Pulverizador → modelo; Pessoa → Atleta →
        indivíduo.
      </p>
      <div
        style={{
      ...
      >
        <div>
          ...
          {categorias.map((c) => (
            <div
              key={c.id}
              onClick={() => setCat(c.id)}
              style={{ ...item(c.id === cat), display: "flex", alignItems: "center", gap: 8 }}
            >
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
            ...
            <button
              ...
              onClick={async () => {
                if (!nCat.trim()) return;
                await api.criarCadastro("categorias", nCat.trim());
                setNCat("");
                recCategorias();
              }}
            >
        ...
        <div>
          ...
          {subs.map((s) => (
            <div key={s.id} onClick={() => setSub(s.id)} style={item(s.id === sub)}>
              {s.nome}
            </div>
          ))}
          {cat && (
            <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
              ...
              <button
                ...
                onClick={async () => {
                  if (!nSub.trim()) return;
                  await api.criarSubcategoria(cat, nSub.trim());
                  setNSub("");
                  setSubs(await api.subcategorias(cat));
                }}
              >
                +
              </button>
          ...
          {sub && (
            <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
              <input
                style={inp}
                value={nObj}
                onChange={(e) => setNObj(e.target.value)}
                placeholder="novo objeto"
              />
              <button
                style={btn}
                onClick={async () => {
                  if (!nObj.trim()) return;
                  await api.criarObjeto(sub, nObj.trim());
                  setNObj("");
                  setObjs(await api.objetos(sub));
                }}
              >
                +
              </button>
