          ...
          <>
            ...
            >
              <Bloco titulo="Por status">
                <Barras dados={d.porStatus} />
              </Bloco>
              <Bloco titulo="Por local">
                <Barras dados={d.porLocal} />
              </Bloco>
              <Bloco titulo="Por área de pesquisa">
                <Barras dados={d.porArea} />
              </Bloco>
              <Bloco titulo="Por safra">
                <Barras dados={d.porSafra} />
              </Bloco>
            </div>

            <Bloco titulo="Checklist do ensaio (avaliações previstas × realizadas)">
              <p style={{ fontSize: 13, color: "#5A6B8C", marginTop: 0 }}>
                ✔ {d.checklist.realizadas} realizadas · ⏳ {d.checklist.pendentes} pendentes · ⚠{" "}
                {d.checklist.atrasadas} atrasadas
                {d.checklist.previstas === 0 &&
                  " — nenhuma avaliação tem data prevista cadastrada ainda."}
              </p>
                  ...
                  >
                    <thead>
                      <tr style={{ textAlign: "left", color: "#7987A1" }}>
                        ...
                        <th style={th}>Avaliação</th>
                        <th style={th}>Experimento</th>
                    ...
                    <tbody>
                      {d.checklist.itens.map((i, idx) => (
                        <tr key={idx} style={{ borderTop: "1px solid #EEF1F6" }}>
                          <td style={td}>
                            {i.estado === "atrasada" ? "⚠ atrasada" : "⏳ pendente"}
                          </td>
                          <td style={td}>{i.avaliacao}</td>
                          <td style={td}>
                            <Link
                              href={`/experimentos/${i.experimentoId}`}
                              style={{ color: "#1F6FB0" }}
                            >
                              {i.experimento}
                            </Link>
  ...
  return (
    <div
      style={{
        ...
        boxShadow: "0 1px 3px rgba(20,27,45,.08)",
      }}
    >
  ...
  return (
    <section
      style={{
        ...
        padding: 16,
        marginTop: 16,
        boxShadow: "0 1px 3px rgba(20,27,45,.06)",
      }}
    >
