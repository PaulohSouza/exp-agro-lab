...
export default function UsuariosPage() {
  ...
  const [f, setF] = useState<{ nome: string; email: string; senha: string; papel: Papel }>({
    nome: "",
    email: "",
    senha: "",
    papel: "analista",
  ...
  async function recarregar() {
    try {
      const [us, deps, unis] = await Promise.all([
        api.listarUsuarios(),
        api.departamentos(),
        api.unidades(),
      ]);
    ...
    <Protected>
      <main style={{ maxWidth: 920, margin: "32px auto", padding: 24 }}>
        ...
        >
          ...
          <select
            value={f.papel}
            onChange={(e) => setF({ ...f, papel: e.target.value as Papel })}
            style={inp}
          >
            {PAPEIS.map((p) => (
        ...
        <p style={{ color: "#7987A1", fontSize: 12 }}>
          Papel, departamento e área/laboratório definem o que cada um enxerga no Painel (RBAC).
          Pesquisador e analista são responsáveis pela coleta.
        </p>
      </main>
    </Protected>
  );
    ...
    <tr style={{ borderBottom: "1px solid #eaecf3" }}>
      ...
      <td style={td}>
        ...
        {sujo && (
          <button
            onClick={salvar}
            style={{ ...btn, marginLeft: 8, padding: "4px 10px", fontSize: 12 }}
          >
            salvar
          </button>
        )}
...
const selp: React.CSSProperties = {
  padding: 6,
  borderRadius: 6,
