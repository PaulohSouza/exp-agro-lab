...
- **Fase B (em curso):** ✅ **Tukey (HSD)** e **Scott-Knott** — amplitude estudentizada `ptukey`/`qtukey` (Gauss-Legendre, espelha o R), Tukey-Kramer p/ n desigual, SK recursivo (λ ~ χ², ν=g/(π−2)); **default Tukey** como no SAGRE; seletor de método na API/UI (`?metodo=`). _Validado vs tabela de Tukey._ ✅ **split-plot (2 erros)** (`anovaSplitPlot`). ✅ **fatorial 2–3 fatores (quali×quali) + desdobramento** (`anovaFatorial`, erro único; desdobra interação dupla em efeitos simples; balanceado ⇒ Type I = Type II). ✅ **transformações** (`transform.ts`): √(x+c), log(x+1) e Box-Cox com λ por **verossimilhança perfilada** (modelo aditivo tratamento[+bloco]); aplicadas na rota da análise (`?transformacao=`), médias retro-transformadas. ✅ **não-paramétrico** (`naoParametrico.ts`): Kruskal-Wallis (H ~ χ²(k−1), correção de empates, post-hoc **Dunn**/Bonferroni) e Friedman (rank no bloco, post-hoc **Nemenyi**); na análise via `?naoParametrico=true` (Kruskal em DIC, Friedman em DBC). ✅ **análise conjunta multi-local** (`conjunta.ts`): Local + Bloco/Local + Tratamento + Local×Trat; locais aleatórios (Tratamento testado pela interação); razão de QM-resíduo entre locais (Cochran ~7×); endpoint `POST /analise/conjunta` + página `/analise-conjunta`. ✅ **Shapiro-Wilk** (`pressupostos.shapiroWilk`, Royston AS R94, validado vs R) + **seleção de rota** (`sugerirRota`: Shapiro+Bartlett → paramétrica/transformação/não-paramétrico), exposta como `rotaSugerida` na análise. **Pendente:** desdobramento da interação tripla, aplicar a rota sugerida automaticamente, e **golden vs SAGRE** (rodar no R e comparar com tolerância — falta o ambiente R).
