...
- **Batch 2 (split-plot + médias/letras)** ✅ **+5 testes** (golden **14 → 19**): **split-plot DBC** (`anovaSplitPlot`, tabela de 2 erros + CV1/CV2 vs `ExpDes.pt::psub2.dbc` / `aov(Error(Bloco/A))`, reusando os dados 3×4) e **médias + agrupamento de Tukey** do caso 1 fator (vs `agricolae::HSD.test` — médias exatas e mesmos pares compartilhando letra).
- **Batch 3 (DIC + não-paramétrico + transformações)** ✅ **+8 testes** (golden **19 → 27**): **DIC 1 fator** (`experimento_grupo_teste`, ANOVA+CV+Tukey), **Kruskal-Wallis** (vs `kruskal.test`: H=23,762) e **Friedman** (vs `friedman.test`: χ²=19,429), **Box-Cox λ** (vs `MASS::boxcox`: λ≈1,021) e **ANOVA na escala transformada** (√/log, F e CV vs `aov`). Analytics nesta branch: **102** (base main 75 + 27 golden; com o PR #21 somado → 107).
- **Falta:** **conjunta multi-local (G×A)** — bloqueado por **dado**: nenhuma planilha do SAGRE-app tem >1 local (todas single-site). Menores: médias/letras em fatorial, assertions no nível do desdobramento (o triplo já tem golden unitário no PR #21, casando com `fat3.dbc`).
