...
- **Macro C — catálogo de atividades** (ação/apontamento). Conceito: Atividade = registro GERAL do experimento.
  ...
  - **Falta C:** C2 domínio (reusar escopo/`podeGerenciarEscopo`; validar apontamento) + testes · C3 API (`ModeloAtividadeModule` + atividades do experimento) · C4 web (catálogo de atividades + aba Atividades no experimento) · **C5 refatorar a colheita** (mover nº linhas/comprimento/área de `AvaliacaoDado` p/ Atividade-Colheita; toca RN-PROD/seed/analytics) — última etapa de C, propositalmente.
