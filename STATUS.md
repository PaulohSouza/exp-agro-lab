...
- **Macro C — catálogo de atividades** (ação/apontamento). Conceito: Atividade = registro GERAL do experimento.
  ...
  - **Falta C:** **C5 refatorar a colheita** (mover nº linhas/comprimento/área de `AvaliacaoDado` p/ Atividade-Colheita; toca RN-PROD/seed/analytics) — última etapa de C, propositalmente. **Plano de migração pronto:** [SDD/04-design-detalhado/08-c5-plano-colheita.md](SDD/04-design-detalhado/08-c5-plano-colheita.md).
...
- **Depois (B):** coleta agrupada (filtro + grupos salvos + lote web/mobile). Ordem: A ✅ → C (falta C5 colheita) → B. Pré-requisitos cruzam avaliações **e** atividades. Testes: **domain 57** + e2e (Playwright Python) em `e2e/` (4 suites, todas passando).
