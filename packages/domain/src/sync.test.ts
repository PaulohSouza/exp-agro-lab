...
describe("sync offline-first", () => {
  it("chaveColeta é idempotente por (avaliacao, parcela, amostra)", () => {
    const k = chaveColeta({ avaliacaoId: "a", parcelaId: "p", numAmostra: 1 });
    expect(k).toBe("a:p:1");
  });
  ...
  it("dedupLote mantém a edição local mais recente por chave", () => {
    const lote: ColetaOffline[] = [
      { avaliacaoId: "a", parcelaId: "p", numAmostra: 1, valorColetado: 5, clientUpdatedAt: 100 },
      { avaliacaoId: "a", parcelaId: "p", numAmostra: 1, valorColetado: 8, clientUpdatedAt: 200 },
      { avaliacaoId: "a", parcelaId: "q", numAmostra: 1, valorColetado: 3, clientUpdatedAt: 100 },
    ];
